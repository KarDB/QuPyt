...
class YamlSequence:
    ...
    def add_pulse(self,
                  ...
                  frequency: float = 0.0,
                  phase: float = 0.0,
                  sequence_blocks: list[str] = ['block_0']) -> None:
        for sequence_block in sequence_blocks:
            ...
            self.pulse_sequence[sequence_block][pulse_channel]['pulse{}'.format(
                self.counter[sequence_block][pulse_channel]
            )] = {
                "start": start,
                "duration": duration,
                "amplitude": amplitude,
...
class ComplexSequence:
    ...
    def gen_phases(self,
        ...
        if seq_type == 'XY8':
            ...
            xy8_phases = [0, np.pi / 2, 0, np.pi / 2,
                          np.pi / 2, 0, np.pi / 2, 0]
    ...
    def write_sequence(self, start: float = 0) -> None:
        ...
        '''
        self.append_pulse(self.channel,
                          start,
...
class ArbitrarySequenceWriter:
    ...
    def add_pulse(
        ...
        sequence_instance.add_pulse(self.channel,
                                    start,
                                    duration,
                                    amplitude=amplitude,
    ...
    def write_sequence(
        ...
        assert num_pulses == len(self.params['durations']) and \
            num_pulses == len(self.params['delays'])-1 and \
            num_pulses == len(
                self.params['mixing_freqs']), 'Uncompatible list lengths.'
    ...
    def prepare_sequence(self, seq_type: str, lock_scaling: float = 1.0) -> Optional[float]:
        ...
        if seq_type == 'XY8':
            ...
            self.params['phases'] = [0, np.pi/2,
                                     0, np.pi/2, np.pi/2, 0, np.pi/2, 0]
            ...
            pi = self.pi
            pi2 = self.pi_half
            self.params['delays'] = [2*self.tau-pi, 2*self.tau, pi2] \
                + [2*self.tau-pi2, 2*self.tau,
                   2*self.tau, 2*self.tau, pi2]*11 \
                + [2*self.tau-pi2, 2*self.tau, pi]
            self.params['durations'] = [pi, pi2, pi2] + \
                [pi, pi, pi, pi2, pi2]*11+[pi, pi]
            self.params['amplitudes'] = [1]*60
            self.params['mixing_freqs'] = [self.res_mix_freq]*60
            self.params['phases'] = [0, 0, -np.pi/2, np.pi, np.pi, 0, 0,
                                     -np.pi/2, np.pi, np.pi, 0, 0, -np.pi/2,
                                     np.pi, np.pi, -np.pi/2, -np.pi/2, 0,
                                     np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, 0,
                                     np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, 0,
                                     np.pi / 2, np.pi/2, -np.pi/2, 0, np.pi/2,
                                     np.pi/2, -np.pi/2, -np.pi/2, 0, np.pi/2,
                                     np.pi/2, -np.pi/2, -np.pi/2, 0, np.pi/2,
                                     np.pi/2, np.pi, np.pi, np.pi/2, 0, 0,
                                     np.pi, np.pi, np.pi/2, 0, 0, np.pi, np.pi,
                                     np.pi/2, 0, 0, -np.pi/2]
            ...
            LG4_freqs = [self.res_mix_freq-det_cd, self.res_mix_freq +
                         det_cd, self.res_mix_freq+det_cd,
                         self.res_mix_freq-det_cd]
            LG4_phases = [np.pi/2-alpha, np.pi+np.pi/2 -
                          alpha, np.pi+np.pi/2+alpha, np.pi/2+alpha]

            self.params['delays'] = [0.0]+[t_cd]*4*self.nLG4 + \
                [self.pi]+[t_cd]*8*self.nLG4+[self.pi]+[t_cd]*4*self.nLG4
            self.params['durations'] = [t_cd]*4*self.nLG4 + [self.pi] + \
                [t_cd]*8*self.nLG4 + [self.pi] + [t_cd]*4*self.nLG4
            self.params['amplitudes'] = [amplitude_cd]*4*self.nLG4 + [1.0] + \
                [amplitude_cd]*8*self.nLG4 + [1.0] + [amplitude_cd]*4*self.nLG4
            self.params['mixing_freqs'] = LG4_freqs*self.nLG4 \
                + [self.res_mix_freq] + LG4_freqs*2*self.nLG4 \
                + [self.res_mix_freq] + LG4_freqs*self.nLG4
            self.params['phases'] = LG4_phases*self.nLG4 + [0] \
                + LG4_phases*2*self.nLG4 + [0] + LG4_phases*self.nLG4
