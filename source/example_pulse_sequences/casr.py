...
def generate_sequence(params: dict):
    ...
    for ps_step in range(pulse_sequence_steps):
        ...
        backup_params = gen_sync(
            ...
            float(params.get('calibration_target_frequency', 1000)),
            float(params.get('ts_start', 1.0)),
            float(params.get('ts_end', 1.0)),
            float(params.get('duration_offset', 0)),
            float(params.get('readout_offset', 1.0)),
            float(params.get('MW_amplitude', 1.0)),
            float(params.get('AOM_frequency', 250e6)),
            float(params.get('readout_phase', 0)),
            float(params.get('global_phase', 0)),
            ps_step,
    )
...
def gen_sync(AWG_frequency: float,
              ...
              sampling_rate: float,
              calibration_target_frequency: float,
              ts_start: float,
              ts_end: float,
